...
const searchFor = async input => (
    await (API + ENDPOINT + encodeURIComponent(input), { json: true })
).body;

module.exports = {
	name: 'urban',
	description: 'searches for word on urban dictionary',
	async execute(message, args) {
		if (args.length < 1) {
            message.channel.send('Please provide a word or phrase to search for.');
        }
        ...
        if (!data || !data.list || data.list.length < 1) {
            message.channel.send('That word could not be found.');
        }
